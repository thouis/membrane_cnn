...
__global__ void run_layers(const float * image, int image_width,
                           int start_row, int start_col, int end_col,
                           const float * __restrict__ filter0,
                           ...
                           float *output0)
{
    ...
    __shared__ unsigned short input[49][128];
    ...
    for (int filter_row = 0; filter_row < 4; filter_row++)
        for (int filter_col = 0; filter_col < 4; filter_col++)
            filter[filter_row][filter_col] = __float2half_rn(FILTER0(filter_row, filter_col, id_outchan, id_maxout));
    ...
    for (int classify_col = start_col; classify_col < end_col; classify_col++) {
        if (input_offset == 64) {
            // shift everything left, load a new sets of 64 floats,
        ...
        //
        // output_row and output_col are the location of the eventual output in the 23x23x32 output.
        for (int output_row=0; output_row < 23; output_row++) {
            for (int output_col=0; output_col < 23; output_col++) {
                // Compute the output for this location given this thread's maxpool/maxout/outchan
                float convsum = BIAS0(output_row, output_col, id_outchan, lane_outchan);
#pragma unroll
                for (int filter_row = 0; filter_row < 4; filter_row++)
#pragma unroll
                    for (int filter_col = 0; filter_col < 4; filter_col++) {
                        float tmp = \
                            input[output_row * 2 + id_maxpool_row + filter_row][output_col * 2 + id_maxpool_col + filter_col + input_offset];
                        convsum += __half2float(filter[filter_row][filter_col]) * tmp;
                    }
                ...
                if (lane_outchan == 0)
                    OUTPUT0(output_row, output_col, id_outchan) = convsum;
            }
        }
        input_offset++;
